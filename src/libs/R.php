<?php
/**
 * Redbean ORM is used in this project.
 *
 * For more information on Redbean please visit : {@link http://www.redbeanphp.com/}.
 *
 * @version    Release: 1.0
 * @author     Gabor de Mooij
 * @copyright  Gabor de Mooij and the RedBeanPHP Community, copyright 2009-2012
 * @license    http://www.redbeanphp.com/extra/license new-BSD
 * @category   ORM
 * @package    Redbean
 * @filesource
 */
...
class RedBean_OODBBean implements IteratorAggregate, ArrayAccess, Countable {
	...
	public function import( $arr, $selection=false, $notrim=false ) {
		...
		if (!$notrim && is_array($selection)) foreach($selection as $k=>$s){
			$selection[$k]=trim($s);
		}
		foreach($arr as $k=>$v) {
			if ($k!='__info') {
